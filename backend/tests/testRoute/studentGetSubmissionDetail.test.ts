...
describe(`using query (6 tests)`, () => {
  describe(`missing crucial params (5 tests)`, () => {
    it('missing submissionId', async () => {
      const respone = await request(app).get('/test/studentGetSubmissionDetail');
    ...
    it('submissionId=null', async () => {
      const respone = await request(app).get('/test/studentGetSubmissionDetail').query({ submissionId: null });
      expect(respone.status).toBe(400);
      expect(respone.body).toEqual({ message: 'Missing `submissionId` parameter!' });
    });
    ...
    it('submissionId=undefined', async () => {
      const respone = await request(app).get('/test/studentGetSubmissionDetail').query({ submissionId: undefined });
      expect(respone.status).toBe(400);
      expect(respone.body).toEqual({ message: 'Missing `submissionId` parameter!' });
    });
  ...
  describe(`crucial params present (1 test)`, () => {
    ...
    it('success!', async () => {
      const respone = await request(app)
        .get('/test/studentGetSubmissionDetail')
...
describe(`query params directly in url (4 tests)`, () => {
  describe(`missing crucial params (3 tests)`, () => {
    ...
    it('submissionId=null', async () => {
      const respone = await request(app).get('/test/studentGetSubmissionDetail?submissionId=null');
      expect(respone.status).toBe(400);
      expect(respone.body).toEqual({ message: 'Missing `submissionId` parameter!' });
    });

    it('submissionId=undefined', async () => {
      const respone = await request(app).get('/test/studentGetSubmissionDetail?submissionId=undefined');
      expect(respone.status).toBe(400);
      expect(respone.body).toEqual({ message: 'Missing `submissionId` parameter!' });
    });
  });
  ...
  describe(`crucial params present (1 test)`, () => {
    ...
    it('success!', async () => {
      const respone = await request(app).get('/test/studentGetSubmissionDetail?submissionId=SUBMIT1&testMode=true');
      expect(respone.status).toBe(200);
      expect(respone.body).toEqual({ message: 'Test passed!' });
    });
  });
