...
describe(`using 'query' (26 tests)`, () => {
  beforeAll(() => {
    jest.spyOn(TestModel.prototype, 'studentGetTestQuestion').mockImplementation(() => 'return mock data');
  });
    ...
    describe(`get a list of questions (5 tests)`, () => {
      it('missing questionId', async () => {
        const respone = await request(app)
          .get('/test/studentGetTestQuestion')
...
describe(`query params directly in url (16 tests)`, () => {
  ...
  describe(`crucial properties present (4 tests)`, () => {
    describe(`get a list of questions (3 tests)`, () => {
      it('missing questionId', async () => {
        const respone = await request(app)
          .get('/test/studentGetTestQuestion?classId=CLASS1&testId=TEST1')
          .query({ testMode: true });
        expect(respone.status).toBe(200);
        expect(respone.body).toEqual({ message: 'Test passed!' });
      });
      ...
      it('questionId=null', async () => {
        const respone = await request(app)
          .get('/test/studentGetTestQuestion?classId=CLASS1&testId=TEST1&questionId=null')
          .query({ testMode: true });
        expect(respone.status).toBe(200);
        expect(respone.body).toEqual({ message: 'Test passed!' });
      });
      ...
      it('questionId=undefined', async () => {
        const respone = await request(app)
          .get('/test/studentGetTestQuestion?classId=CLASS1&testId=TEST1&questionId=undefined')
          .query({ testMode: true });
        expect(respone.status).toBe(200);
        expect(respone.body).toEqual({ message: 'Test passed!' });
      });
    ...
    describe(`get a specific question (1 tests)`, () => {
      it('get a specific question successfully', async () => {
        const respone = await request(app)
          .get('/test/studentGetTestQuestion?classId=CLASS1&testId=TEST1&questionId=QUESTION1')
          .query({ testMode: true });
        expect(respone.status).toBe(200);
        expect(respone.body).toEqual({ message: 'Test passed!' });
      });
  ...
  describe(`crucial properties not present (12 tests)`, () => {
    describe(`trying to get a list of questions (6 tests)`, () => {
      it('missing classId', async () => {
        const respone = await request(app).get('/test/studentGetTestQuestion?testId=TEST1').query({ testMode: true });
        expect(respone.status).toBe(400);
        expect(respone.body).toEqual({ message: 'Missing `classId` parameter!' });
      });
      ...
      it('classId=null', async () => {
        const respone = await request(app)
          .get('/test/studentGetTestQuestion?classId=null&testId=TEST1')
          .query({ testMode: true });
        expect(respone.status).toBe(400);
        expect(respone.body).toEqual({ message: 'Missing `classId` parameter!' });
      });
      ...
      it('classId=undefined', async () => {
        const respone = await request(app)
          .get('/test/studentGetTestQuestion?classId=undefined&testId=TEST1')
          .query({ testMode: true });
        expect(respone.status).toBe(400);
        expect(respone.body).toEqual({ message: 'Missing `classId` parameter!' });
      });
      ...
      it('missing testId', async () => {
        const respone = await request(app).get('/test/studentGetTestQuestion?classId=CLASS1').query({ testMode: true });
        expect(respone.status).toBe(400);
        expect(respone.body).toEqual({ message: 'Missing `testId` parameter!' });
      });
      ...
      it('testId=null', async () => {
        const respone = await request(app)
          .get('/test/studentGetTestQuestion?classId=CLASS1&testId=null')
          .query({ testMode: true });
        expect(respone.status).toBe(400);
        expect(respone.body).toEqual({ message: 'Missing `testId` parameter!' });
      });
      ...
      it('testId=undefined', async () => {
        const respone = await request(app)
          .get('/test/studentGetTestQuestion?classId=CLASS1&testId=undefined')
          .query({ testMode: true });
        expect(respone.status).toBe(400);
        expect(respone.body).toEqual({ message: 'Missing `testId` parameter!' });
      });
    ...
    describe(`trying to get a specific question (6 tests)`, () => {
      it('missing classId', async () => {
        const respone = await request(app)
          .get('/test/studentGetTestQuestion?testId=TEST1&questionId=QUESTION1')
          .query({ testMode: true });
        expect(respone.status).toBe(400);
        expect(respone.body).toEqual({ message: 'Missing `classId` parameter!' });
      });
      ...
      it('classId=null', async () => {
        const respone = await request(app)
          .get('/test/studentGetTestQuestion?classId=null&testId=TEST1&questionId=QUESTION1')
          .query({ testMode: true });
        expect(respone.status).toBe(400);
        expect(respone.body).toEqual({ message: 'Missing `classId` parameter!' });
      });
      ...
      it('classId=undefined', async () => {
        const respone = await request(app)
          .get('/test/studentGetTestQuestion?classId=undefined&testId=TEST1&questionId=QUESTION1')
          .query({ testMode: true });
        expect(respone.status).toBe(400);
        expect(respone.body).toEqual({ message: 'Missing `classId` parameter!' });
      });
      ...
      it('missing testId', async () => {
        const respone = await request(app)
          .get('/test/studentGetTestQuestion?classId=CLASS1&questionId=QUESTION1')
          .query({ testMode: true });
        expect(respone.status).toBe(400);
        expect(respone.body).toEqual({ message: 'Missing `testId` parameter!' });
      });
      ...
      it('testId=null', async () => {
        const respone = await request(app)
          .get('/test/studentGetTestQuestion?classId=CLASS1&testId=null&questionId=QUESTION1')
          .query({ testMode: true });
        expect(respone.status).toBe(400);
        expect(respone.body).toEqual({ message: 'Missing `testId` parameter!' });
      });
      ...
      it('testId=undefined', async () => {
        const respone = await request(app)
          .get('/test/studentGetTestQuestion?classId=CLASS1&testId=undefined&questionId=QUESTION1')
          .query({ testMode: true });
        expect(respone.status).toBe(400);
        expect(respone.body).toEqual({ message: 'Missing `testId` parameter!' });
      });
